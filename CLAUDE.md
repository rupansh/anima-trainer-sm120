...
- **T-LoKr:** `lokr.variant = "tlokr"` applies Vanilla T-LoRA's timestep rank schedule to a factorized large Kronecker operand while retaining LoKr's small full operand. The default minimum rank is 50% at maximum noise and grows to full rank at the clean endpoint. The production path is structured rather than delta-materializing: TE block-FP8 large-factor GEMMs, fused Triton mask/small-factor forward+dgrad, fused fp32-reduction wgrad, recompute-based custom autograd, and factor slicing for uniform-timestep sampling. See `tlokr.py` and `tlokr_kernels.py`.
...
- Resumable trainer state lives under `<cache_db>/_training_state/` and is published only at adapter-checkpoint boundaries. It includes LoKr/optimizer state, epoch/global step, and Python/NumPy/CPU/CUDA RNG state. Restoration is gated on model, exact cached-input, semantic-config, and runtime fingerprints; operational cadence/worker/target-epoch changes are allowed. See `training_state.py`.
- DataLoader workers persist across epochs. An abrupt process death falls back permanently to deterministic `num_workers=0` replay after the last completed batch; wrapped dataset/cache exceptions still fail normally. See `resilient_loader.py`.
